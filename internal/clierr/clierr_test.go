...
func TestClassifyConstructors(t *testing.T) {
	tests := []struct {
		...
		{name: "usage", err: Usage("bad flag"), wantCode: "USAGE_ERROR", wantExit: ExitUsage},
		{name: "not found", err: NotFound("cluster %q not found", "a"), wantCode: "RESOURCE_NOT_FOUND", wantExit: ExitNotFound},
		{name: "conflict", err: Conflict("ambiguous"), wantCode: "RESOURCE_CONFLICT", wantExit: ExitConflict},
		{name: "unclassified", err: errors.New("boom"), wantCode: "CLI_ERROR", wantExit: ExitGenericFailure},
		{name: "wrapped classified error still classifies", err: fmt.Errorf("wrap: %w", Usage("bad flag")), wantCode: "USAGE_ERROR", wantExit: ExitUsage},
	}
...
func TestHTTPStatusErrorClassification(t *testing.T) {
	tests := []struct {
		...
		{status: 400, wantCode: "VALIDATION_ERROR", wantExit: ExitValidation},
		{status: 422, wantCode: "VALIDATION_ERROR", wantExit: ExitValidation},
		{status: 401, wantCode: "AUTHENTICATION_FAILED", wantExit: ExitAuthentication},
		{status: 403, wantCode: "PERMISSION_DENIED", wantExit: ExitAuthorization},
		{status: 404, wantCode: "RESOURCE_NOT_FOUND", wantExit: ExitNotFound},
		{status: 409, wantCode: "RESOURCE_CONFLICT", wantExit: ExitConflict},
		{status: 429, wantCode: "RATE_LIMITED", wantExit: ExitRateLimited, wantRetryable: true},
		{status: 500, wantCode: "API_SERVER_ERROR", wantExit: ExitServer, wantRetryable: true},
