...
- errors are classified by `internal/clierr` (stable exit code, machine-readable `code`, `retryable`, human-readable `hint`, and — for HTTP-backed errors, via the `HasRequestID` interface — `request_id` from the API's `X-Request-Id` header), always, regardless of output mode; when the requested output mode (resolved from raw argv/env via `config.OutputModeFromArgs`, since an error can occur before a Runtime exists) is `json`/`jsonl`, `main.go` renders them via `output.WriteJSONError` instead of plain-text `log.Print`+hint/request-id lines, following the same pretty-vs-compact split. `internal/cli/app.go`'s `New()` sets a no-op `ExitErrHandler` so urfave/cli's own internally generated errors (e.g. an unrecognized command) flow through this same path instead of calling `os.Exit` early. See README.md's "Machine-Readable Output" section (PSD-9883)
